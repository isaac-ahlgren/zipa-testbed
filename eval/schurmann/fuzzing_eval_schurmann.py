...
def generate_bits(sf, bit_length, window_length, band_length, max_samples, sampling_freq, antialias_freq):
    freq_bin_len = (sampling_freq / 2) / (int(window_length / 2) + 1)
    antialias_bin = int(np.floor(antialias_freq / freq_bin_len))
    samples_per_key = (
            math.ceil(
                ((bit_length) / int(antialias_bin / band_length))
                + 1
            )
            * window_length
        )
    ...
    for i in range(number_of_keys):
        ...
        key = shurmann_wrapper_func(samples, window_length, band_length, sampling_freq, antialias_freq)
...
def experiment(data_from_devices, ids, iterations, max_samples):
    ...
    for i in range(iterations):
        ...
        for sf in data_from_devices:
            bits, samples_per_key = generate_bits(sf, bit_length, window_length, band_len, max_samples, MICROPHONE_SAMPLING_RATE, ANTIALIASING_FILTER)
