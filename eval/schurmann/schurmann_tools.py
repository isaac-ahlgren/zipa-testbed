...
import os
import math
sys.path.insert(1, os.getcwd() + "/../../src/") # Gives us path to Schurmann and Siggs algorithm in /src
...
def schurmann_calc_sample_num(key_length, window_length, band_length, sampling_freq, antialias_freq):
    freq_bin_len = (sampling_freq / 2) / (int(window_length / 2) + 1)
    antialias_bin = int(np.floor(antialias_freq / freq_bin_len))
    return (math.ceil(((key_length) / int(antialias_bin / band_length)) + 1)
           * window_length
           )
