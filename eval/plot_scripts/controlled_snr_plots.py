...
from fastzip import (  # noqa: E402
    controlled_signal_plus_noise_fastzip,
    goldsig_plus_noise_eval_fastzip,
)
from iotcupid import (  # noqa: E402
    controlled_signal_plus_noise_iotcupid,
    goldsig_plus_noise_eval_iotcupid,
)
from miettinen import (  # noqa: E402
    controlled_signal_plus_noise_miettinen,
    goldsig_plus_noise_eval_miettinen,
)
from perceptio import (  # noqa: E402
    controlled_signal_plus_noise_perceptio,
    goldsig_plus_noise_eval_perceptio,
)
from schurmann import (  # noqa: E402
    controlled_signal_plus_noise_schurmann,
    goldsig_plus_noise_eval_schurmann,
)
...
def gen_gold_sig_data(snr_levels):
    goldsig_eval_funcs = [
        goldsig_plus_noise_eval_schurmann.main,
...
def gen_controlled_sig_data(snr_levels):
    controlled_sig_eval_funcs = [
        controlled_signal_plus_noise_schurmann.main,
    ...
    legit_ber_matrix, adv_ber_matrix = test_snr_levels(
        controlled_sig_eval_funcs, snr_levels
    )
    return legit_ber_matrix, adv_ber_matrix
...
def main(snr_levels):
    ...
    controlled_sig_legit_ber_matrix, controlled_sig_adv_ber_matrix = (
        gen_controlled_sig_data(snr_levels)
    )

    goldsig_legit_mat = convert_to_data_frame(goldsig_legit_ber_matrix, snr_levels)
    controlled_legit_mat = convert_to_data_frame(
        controlled_sig_legit_ber_matrix, snr_levels
    )

    goldsig_adv_mat = convert_to_data_frame(goldsig_adv_ber_matrix, snr_levels)
    controlled_adv_mat = convert_to_data_frame(
        controlled_sig_adv_ber_matrix, snr_levels
    )
    ...
    plot_matrix(
        goldsig_legit_mat,
        "Protocol BER by SNR Level Using Golden Signal",
        saveplot=True,
        filename="./plot_data/gold_sig_legit_snr.png",
    )
    plot_matrix(
        controlled_legit_mat,
        "Protocol BER by SNR Level Using Controlled Signal",
        saveplot=True,
        filename="./plot_data/controlled_sig_legit_snr.png",
    )
    plot_matrix(
        goldsig_adv_mat,
        "Protocol BER by SNR Level Using Adversary Signal",
        saveplot=True,
        filename="./plot_data/gold_sig_adv_snr.png",
    )
    plot_matrix(
        controlled_adv_mat,
        "Protocol BER by SNR Level Using Controlled Adversary Signal",
        saveplot=True,
        filename="./plot_data/controlled_sig_adv_snr.png",
    )
