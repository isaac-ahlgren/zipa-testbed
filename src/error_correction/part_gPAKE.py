...
class GPAKE:
    ...
    def generate_shared_key_host(
        self, priv_key, peer_pub_key_bytes, local_device_id, peer_device_id
    ):
        ...
        peer_pub_key = ec.EllipticCurvePublicKey.from_encoded_point(
            self.curve, peer_pub_key_bytes
        )
        shared_key = priv_key.exchange(ec.ECDH(), peer_pub_key)

        info = (
            local_device_id.encode()
            + peer_device_id.encode()
    ...
    def generate_shared_key_device(
        self, priv_key, peer_pub_key_bytes, local_device_id, peer_device_id
    ):
    ...
    def encode(self, symm_key, pub_key_bytes, iv):
        ...
        encryptor = Cipher(
            self.algo(symm_key), self.mode(iv), default_backend()
        ).encryptor()
    ...
    def handle_device_public_key_exchange(self, conn, priv_key, password, device_id):
        ...
        session_id_received = json.loads(msg[1].decode())  # noqa: F841
        ...
        return self.generate_shared_key_device(
            priv_key, device_pub_key_bytes, device_id, device_id_received
        )
    ...
    def receive_and_decrypt_random_value(self, conn, shared_key):
        ...
        decrypted_random_value = self.decode(
            shared_key, enc_random_value_received, iv_received
        )
    ...
    def handle_host_public_key_exchange(
        self, conn, priv_key, pub_key, password, device_id, session_idx
    ):
        ...
        session_id_bytes = json.dumps(session_id).encode()
        send_pake_msg(
            conn, [device_id.encode(), session_id_bytes, enc_device_pub_key, iv]
        )

        return self.generate_shared_key_host(
            priv_key, host_pub_key_bytes, device_id, host_device_id
        )
    ...
    def host_protocol(self, grouped_events, passwords, conn):
        ...
        for i in range(len(grouped_events)):
            ...
            self.send_encrypted_public_key(
                conn, device_id, i, pub_keys[i], passwords[i]
            )
            # Step 3-7: Receive device's public keys, decrypt them, and exchange random values
            shared_key = self.handle_device_public_key_exchange(
                conn, priv_keys[i], passwords[i], device_id
            )
            ...
            received_random_value = self.receive_and_decrypt_random_value(
                conn, shared_key
            )
    ...
    def device_protocol(self, grouped_events, passwords, conn, device_id):
        ...
        final_random_values = []
        for i in range(len(grouped_events)):
            shared_key = self.handle_host_public_key_exchange(
                conn, priv_keys[i], pub_keys[i], passwords[i], device_id, i
            )
            received_random_value = self.receive_and_decrypt_random_value(
                conn, shared_key
            )
