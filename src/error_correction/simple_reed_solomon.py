...
class SimpleReedSolomonObj:
    ...
    def bytes_to_array(self, bs):
        ...
        for i in range(total_blocks):
            block_in_bytes = bs[i*self.block_byte_size : (i+1)*self.block_byte_size]
    ...
    def rs_correct_msg(self, msg_in, nsym, generator, fcr=0, erase_pos=None, only_erasures=False):
        ...
        if len(msg_in) > self.field_charac:
            raise ValueError("Message is too long (%i when max is %i)" % (len(msg_in), self.field_charac))
    ...
    def rs_find_error_locator(self, synd, nsym, erase_loc=None, erase_count=0):
        ...
        for i in range(nsym-erase_count): # generally: nsym-erase_count == len(synd), except when you input a partial erase_loc and using the full syndrome instead of the Forney syndrome, in which case nsym-erase_count is more correct (len(synd) will fail badly with IndexError).
            ...
            for j in range(1, len(err_loc)):
                delta ^= self.gf_mul(err_loc[-(j+1)], synd[K - j]) # delta is also called discrepancy. Here we do a partial polynomial multiplication (ie, we compute the polynomial multiplication only for the term of degree K). Should be equivalent to brownanrs.polynomial.mul_at().
    ...
    def gf_poly_mul(self, p, q): # simple equivalent way of multiplying two polynomials without precomputation, but thus it's slower
            ...
            if coef != 0: # log(0) is undefined, so we need to avoid that case explicitly (and it's also a good optimization).
                for j in range(1, len(divisor)): # in synthetic division, we always skip the first coefficient of the divisior,
                    ...
                    if divisor[j] != 0: # log(0) is undefined
                        msg_out[i + j] ^= self.gf_mul(divisor[j], coef) # equivalent to the more mathematically correct
