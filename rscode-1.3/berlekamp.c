...
static void init_gamma(int gamma[], struct ReedSolomon_Instance* rs);
static void compute_modified_omega (struct ReedSolomon_Instance* rs);
static void mul_z_poly (int src[], int max_deg);
...
/* given Psi (called Lambda in Modified_Berlekamp_Massey) and synBytes,
  ...
  */
void
compute_modified_omega (struct ReedSolomon_Instance* rs)
{
  int i, npar = rs->npar, max_deg = rs->max_deg;
  int product[max_deg*2];
  int* Lambda = rs->Lambda;
  int* synBytes = rs->synBytes;
	int* Omega = rs->Omega;
...
void
mult_polys (int dst[], int p1[], int p2[], int max_deg)
{
  int i, j;
  int tmp1[max_deg*2];
...
{
  int i;
  for (i = max_deg-1; i > 0; i--) src[i] = src[i-1];
  src[0] = 0;
}
